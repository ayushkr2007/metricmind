...
// ============================================================
// DEMO DATA (fallback when Cube is unavailable)
// ============================================================
const DEMO_DATA = {
    sales_by_category: [
...
async function queryCube(question: string): Promise<AgentResponse | null> {
    try {
        ...
        if (!cubeApiUrl || !cubeApiToken) {
            console.log("Cube credentials missing – using demo data");
            return null;
        }

        const cube = cubejs(cubeApiToken, { apiUrl: cubeApiUrl });
        const q = question.toLowerCase();
        ...
        if (q.includes("sales") && (q.includes("category") || q.includes("product"))) {
            ...
            // Default: KPI summary
            query = {
                measures: ["superstore_view.sales", "superstore_view.profit", "superstore_view.count"],
            };
        ...
        // Clean up keys (remove cube prefix)
...
async function runAgent(question: string): Promise<AgentResponse> {
    ...
    // Enhance with AI explanation (Groq)
    try {
        ...
        const prompt = ChatPromptTemplate.fromMessages([
            [
                "system",
                `You are MetricMind, an AI business intelligence assistant for a retail Superstore dataset.
